...
class Secp256r1VerificationKey: IVerificationKey {
  ...
  init(_ publicKey: String) {
    ...
    self.secpVerifier = Secp256r1Verifier()
  }
...
func testAccess(
  ...
  try await response.verify(eccVerifier, try await responseVerificationKey.public())
...
final class IntegrationTests: XCTestCase {
  func testCompletesAuthFlows() async throws {
    ...
    let recoveryHash = try await hasher.sum(try await recoverySigner.public())
